...
import { Order, PaymentMethod, StudentStackParamList } from "@/types";
...
const PaymentScreen: React.FC<PaymentScreenProps> = ({ navigation, route }) => {
  ...
  const processPayment = async (data: PaymentForm) => {
    ...
    try {
      ...
      const result = await PaymentService.getInstance().processOrderPayment(
        order,
        data.paymentMethod,
        customerData,
      );
  ...
  const monitorPaymentStatus = async (transactionId: string) => {
    ...
    const checkStatus = async (): Promise<void> => {
      try {
        const status =
          await PaymentService.getInstance().checkPaymentStatus(transactionId);
  ...
  const renderPaymentMethods = () => {
    const methods = PaymentService.getInstance().getSupportedPaymentMethods();
      ...
      <Card style={styles.methodsCard}>
        <Card.Content>
          ...
          >
            {methods.map((method) => {
              const methodInfo =
                PaymentService.getInstance().getPaymentMethodInfo(method);
      ...
      <Card style={styles.summaryCard}>
        <Card.Content>
          ...
          <View style={styles.summaryRow}>
            ...
            <Text>
              {PaymentService.getInstance().formatAmount(
          ...
          <View style={styles.summaryRow}>
            ...
            <Text>
              {PaymentService.getInstance().formatAmount(order.delivery_fee)}
            </Text>
          </View>
          ...
          <View style={styles.summaryRow}>
            ...
            <Text>
              {PaymentService.getInstance().formatAmount(order.service_fee)}
            </Text>
          </View>
          ...
          {paymentFees.fees > 0 && (
            <View style={styles.summaryRow}>
              ...
              <Text>
                {PaymentService.getInstance().formatAmount(paymentFees.fees)}
              </Text>
            </View>
          )}
          ...
          <View style={[styles.summaryRow, styles.totalRow]}>
            ...
            <Text style={styles.totalText}>
              {PaymentService.getInstance().formatAmount(paymentFees.total)}
            </Text>
          </View>
        </Card.Content>
      </Card>
      ...
      >
        ...
        <Surface style={styles.actionContainer}>
          ...
          >
            {selectedMethod === "cash"
              ? "Confirm Order"
              : `Pay ${PaymentService.getInstance().formatAmount(
                  PaymentService.getInstance().calculatePaymentFees(
                    amount,
                    selectedMethod,
                  ).total,
                )}`}
